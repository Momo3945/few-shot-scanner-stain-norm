#!/usr/bin/env python3
"""
train_p3_07b_lora_sdxl.py -- P3-07b: bounded >50-pair training follow-up to
P3-07 (native-1024 SDXL transfer of P1-10's source-conditioned colour LoRA +
fresh 6-channel ControlNet).

Motivation (see tickets/PHASE3-TICKETS.md P3-07, D1-D5): P3-07's held-out
evaluation found native 1024 resolution closes most of SDXL's structural
gap vs SD1.5 but flips colour recovery negative on every slide
...
controlnet_conditioning_scale sweep, D5 colour-LoRA-scale sweep) ruled out a
metric artefact, a mis-trained LoRA, a fixable conditioning-channel
imbalance, and every inference-only knob available on the existing ≤50-pair
checkpoint. The remaining hypothesis (H3) is that native-1024 crops are a
harder/more heterogeneous colour-learning problem than 512, and that ≤50
paired A03/H03 crops are simply insufficient data to learn it at this
resolution.

**This script is IDENTICAL in architecture and every hyperparameter to
train_colour_translation_lora_sdxl.py (P3-06/P3-07) -- same SDXL base, same
frozen-UNet + LoRA (rank 8, target_modules to_k/to_q/to_v/to_out.0), same
fresh 6-channel source-conditioning ControlNet (ControlNetModel.from_unet,
conditioning_channels=6, only the conditioning encoder + zero-conv output
layers trainable), same leak-checked A03/H03-only build_pairs(), same
leave-one-frame-out val split, same 4000-step budget, same resolution=1024.
The ONLY variable that changes is training-set size: this script's
--pairs-dir points at pairs/train_1024_full96/ (all 96 non-overlapping
tissue-passing A03/H03 1024 crop candidates measured in job 46384) instead
of P3-07's capped pairs/train_1024/ (50 pairs) -- pairs/train_1024's 50 are
a deterministic subset of these 96 (same --seed 0 round-robin selection,
just uncapped), so this is a strict data superset of P3-07's training set,
not a different sample.**

**Scope note -- read before citing this run anywhere:** `docs/proposal_draft(6).tex`'s
H1/RQ1 is a literal, capped hypothesis: "≤50 coordinate-corresponding
A03/H03 crop pairs are sufficient to train the scanner-colour LoRA... from
ONE slide pair". P3-07 (≤50 pairs) is the run that answers that formal
claim, regardless of what this script finds. **This script (P3-07b) is a
supplementary >50-pair follow-up, explicitly NOT evidence for or against
the ≤50-pair H1 claim** -- it exists only to test H3 (is 1024 an
insufficient-data problem, separate from the few-shot hypothesis itself).
Report it as such; do not fold its result into P3-07's H1-relevant numbers.

New, separate checkpoint tag (lora/a2h_cond_r8_sdxl_1024_full96/) -- never
touches lora/a2h_cond_r8_sdxl_1024/ (P3-07) or any other existing checkpoint.

Everything below this point (leak check, spatial-jitter tolerance,
validation-split rationale, SDXL added_cond_kwargs wiring for both
ControlNet and UNet) is copied verbatim from
train_colour_translation_lora_sdxl.py -- see that script's docstring for
the full explanation of each design choice; not repeated here since nothing
about them changes for this run.

Usage
-----
    # smoke test (env/loop check only -- the architecture itself is already
    # validated by P3-07's own smoke/overfit-8/ablation controls; this run
    # only changes pair count, so a smoke test is the only new-script
    # sanity check needed before the real run):
    python train_p3_07b_lora_sdxl.py --pairs-dir <pairs>/train_1024_full96 \
        --direction A2H --output-dir <out>/p3_07b_smoke --smoke

    # real run:
    python train_p3_07b_lora_sdxl.py --pairs-dir <pairs>/train_1024_full96 \
        --direction A2H --rank 8 --train-steps 4000 --resolution 1024 \
        --output-dir <out>/a2h_cond_r8_sdxl_1024_full96
...
# ----------------------------------------------------------------------
# Pure helpers -- IDENTICAL to train_colour_translation_lora_sdxl.py,
# copied verbatim (this script changes training-set size only, nothing
# about the leak check, hashing, or CLI surface).
# ----------------------------------------------------------------------
def build_pairs(pairs_dir: str) -> list[dict]:
    ...
    if bad_slides:
        raise SystemExit(f"LEAK CHECK FAILED: unexpected slide(s) {bad_slides} in {pairs_dir} "
                         f"(P3-07/P3-07b scope is A03/H03 only).")
    return pairs
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="P3-07b: >50-pair supplementary follow-up training run (same architecture "
                    "as P3-06/P3-07, more A03/H03 1024 pairs). NOT evidence for/against the "
                    "formal <=50-pair H1 claim -- see module docstring.")
    ap.add_argument("--pairs-dir", required=True,
                    help="Directory with *_aperio.png / *_hamamatsu.png pairs (A03/H03 only). "
                         "Expected: pairs/train_1024_full96 (96 pairs) for the real P3-07b run.")
    ap.add_argument("--direction", choices=["A2H", "H2A"], required=True,
                    help="A2H: source=Aperio, target=Hamamatsu. In scope for P3-07b, matching "
                         "P3-06/P3-07's own descope of H2A.")
    ...
    ap.add_argument("--resolution", type=int, default=1024,
                    help="pairs/train_1024_full96 crops are already exactly this size -- "
                         "honest SDXL micro-conditioning value, matching P3-07 (not 512).")
    ...
    ap.add_argument("--train-steps", type=int, default=4000,
                    help="Fixed at P3-07's step count (job 46552) for direct comparability.")
    ...
    ap.add_argument("--overfit-n", type=int, default=0,
                    help="Restrict to the first N pairs and train longer. Not required for "
                         "P3-07b (the architecture itself was already validated at ≤50 pairs by "
                         "P3-07's own overfit-8 + ablation controls) -- kept for parity/debugging.")
...
def main():
    ...
    # (identical to train_colour_translation_lora_sdxl.py)
    # ------------------------------------------------------------------
    ...
    print(f"P3-07b -- Direction {args.direction}: {len(all_pairs)} pairs, "
    ...
    # ControlNet cloned from it -> THEN the LoRA. Identical to P3-06/P3-07.
    # ------------------------------------------------------------------
    ...
    # forward/backward every step -- gradient checkpointing on the UNet keeps
    # this inside a 24GB RTX 3090.
    ...
    # SDXL micro-conditioning: honest values for this data -- resolution=1024,
    # matching P3-07 (not spoofed to 512).
    ...
    def run_step(target_pixels, control_images, generator=None):
        ...
        with torch.autocast(device_type="cuda", dtype=amp_dtype,
            ...
            # SDXL's ControlNetModel needs added_cond_kwargs too, not just the UNet.
    ...
    with open(out_dir / "training_config.json", "w") as fh:
        json.dump(vars(args) | {"n_pairs": len(all_pairs), "pairs_hash": h,
                                ...
                                "supplementary_not_h1_evidence": True},
                  fh, indent=2)
    print(f"\nDone. Final LoRA + ControlNet in {out_dir/'final'}. Config + loss log written.")
    print("P3-07b is a supplementary >50-pair follow-up -- NOT evidence for/against the "
          "formal <=50-pair H1 claim (see module docstring).")
