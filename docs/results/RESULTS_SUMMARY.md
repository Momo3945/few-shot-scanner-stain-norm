...
fusion) is CONFIRMED at full held-out scale — the first configuration in
this project's entire history (37+ configs across A0–A5, P1-10/11/12/13,
and SDXL) to clear classical stain-normalisation baselines on SSIM.** Fuses
raw source-image detail back into P1-11's output *after* generation,
entirely outside the UNet — deliberately sidesteps P1-17's failure mode,
since the fusion never re-enters the shared diffusion computation. The
intent: let diffusion supply only the colour/style transformation while the
untouched Aperio source supplies the fine structure, instead of asking the
VAE to resynthesise both.
...
crops: +13.4%; 50 crops: +15.6%). Beaten controls: full luminance swap (F1)
and a plain alpha-blend (F2) both underperform F3 on the SSIM/LAB tradeoff.
A visual artifact check found F3 indistinguishable from raw source at
native and zoomed scale — no haloing, ringing, or gamut clipping.

**Full 496-crop, 5-slide held-out result (2026-08-28), F3 frozen config:**
...
just an 80-crop subset. **Four of five held-out slides (A08, A09, A13, A16)
individually clear the classical range outright** — only A06, this
project's persistent colour-gap outlier, falls narrowly short, and only of
the weakest baseline (0.617 vs. 0.628). Every per-slide recovery Δlab is
positive. Against P1-11's own full-496 result: SSIM **0.4960 → 0.729
(+0.233 absolute / +47% relative)**, colour recovery Δlab +7.81 vs. P1-11's
+8.74 (~89% retained) — a real, modest tradeoff for the SSIM gain, not a
revert-to-raw (confirmed both by the retention ratio and a direct pixel-diff
sanity check: fused output vs. raw Aperio = 3.09/255, vs. real Hamamatsu =
53.09/255, essentially matching raw's own 53.74/255 gap to target).

**The path to this number had two real bugs, both caught before being
trusted, not swept under the rug:**
1. `score_outputs.py`'s baseline-weighting function used the wrong
   per-slide crop counts for subset runs, initially showing a spurious
   −22.14 pooled recovery on an 80-crop check despite positive per-slide
   deltas — fixed and pushed (`be1f4f8`).
2. A first attempt to scale to the full 496 crops cheaply reused a
   pre-existing eval directory as the raw-Aperio reference; that
   directory's reference field turned out to be the real Hamamatsu target,
   not Aperio, silently leaking ground truth into the fusion input and
   producing an invalid SSIM≈0.998. Caught by the number being implausible
   on its face, confirmed by direct pixel comparison, root-caused by
   checksumming the reference against known-good Aperio/Hamamatsu
   checksums. **The 0.998 figure is invalid and was discarded** — the 0.729
   figure above is a fresh, genuine identity-mode run (job 47361),
   checksum-verified against the known-good raw-Aperio hash before scoring.

**Not yet done**: the F1/F2 comparison at held-out scale (only run on
internal validation so far); a Relative Dice/HoVer-Net structural check
(the ticket's optional final-config-only check — now warranted, since the
full-496 result clears the ticket's own acceptance bar). Full narrative,
grid data, and both bug writeups: `tickets/
